...
class DriveVLMT5(nn.Module):
    ...
    def generate(self, text_enc, imgs, lidar=None):
        ...
        output_ids = self.model.generate(attention_mask=attention_mask, decoder_input_ids=decoder_input_ids,inputs_embeds=merged_embedding, max_length=512,num_beams=4,early_stopping=True)
  
        #Temperature = 0.3 (with sampling)
        #output_ids = self.model.generate(attention_mask=attention_mask, decoder_input_ids=decoder_input_ids, inputs_embeds=merged_embedding, max_length=512, do_sample=True, temperature=0.3, top_k=0)

        #Top-p Sampling (top_p = 0.9)
        #output_ids = self.model.generate(attention_mask=attention_mask, decoder_input_ids=decoder_input_ids, inputs_embeds=merged_embedding, max_length=512, do_sample=True, top_p=0.9, top_k=0)

        #Max Output Length (max_length = 256)
        #output_ids = self.model.generate(attention_mask=attention_mask, decoder_input_ids=decoder_input_ids, inputs_embeds=merged_embedding, max_length=256, early_stopping=True)
